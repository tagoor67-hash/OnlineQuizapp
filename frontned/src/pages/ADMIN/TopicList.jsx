...
const TopicList = () => {
  ...
  const handleUpdateTopic = async (topicId) => {
    ...
    try {
      const response = await fetch(`${import.meta.env.VITE_API_BASE_URL || 'http://localhost:5000'}/api/courses/updatetopic/${courseName}/${topicId}`, {
        method: "PUT",
  ...
  const handleAddTopic = async () => {
    ...
    try {
      const response = await fetch(`${import.meta.env.VITE_API_BASE_URL || 'http://localhost:5000'}/api/courses/addcourse/${courseName}/topics`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
      ...
      const updatedResponse = await fetch(`${import.meta.env.VITE_API_BASE_URL || 'http://localhost:5000'}/api/courses/topics/${courseName}`);
